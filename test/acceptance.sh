...
# Isolate from the dev/CI user's environment so install paths and tmux
# config can't leak into (or out of) the test.
...
tmux -f /dev/null -S "$isotmux" new-session -d -s iso 'sleep 30' 2>/dev/null || true
