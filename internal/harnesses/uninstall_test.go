...
func TestUninstall_PrunesEmptyDirs(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
	m := Manifest{
		Name: "demo", Command: "demo",
		Install: Install{Files: []File{
			{Src: "a.sh", Dst: "~/.config/demo/nested/deep/a.sh", Mode: "0644"},
		}},
	}
	fs := fstest.MapFS{"demo/a.sh": {Data: []byte("x")}}
	ops, _ := BuildPlan(m, fs)
	r, _ := Apply("demo", ops)
	if warns := Uninstall(r); len(warns) != 0 {
		t.Fatalf("warns: %v", warns)
	}
	// Every directory we created should be gone, up to ~ (HOME).
	for _, p := range []string{
		filepath.Join(tmp, ".config/demo/nested/deep"),
		filepath.Join(tmp, ".config/demo/nested"),
		filepath.Join(tmp, ".config/demo"),
		filepath.Join(tmp, ".config"),
	} {
		if _, err := os.Stat(p); !os.IsNotExist(err) {
			t.Errorf("expected pruned: %s (err=%v)", p, err)
		}
	}
	// HOME itself must survive.
