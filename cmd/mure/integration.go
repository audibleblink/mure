...
func integrationInstall(ms []harnesses.Manifest, name string, stdout, stderr *os.File) int {
	...
	if !m.Capabilities.Status || !m.Capabilities.Result {
		fmt.Fprintf(stderr, "warning: %s is degraded (status=%v result=%v); capture-pane fallback is not yet implemented, so these signals will be silent\n",
			name, m.Capabilities.Status, m.Capabilities.Result)
	}
