...
- [ ] ~~`internal/harnesses/fallback.go`: capture-pane status heuristic.~~ **DEFERRED** (see PRD §10). No `Degraded` field on `sock.AgentSnapshot`, no classifier, no poll loop. All shipped harnesses provide hook-based status/result; fallback has no current consumer.
- [ ] ~~Fallback lives in `internal/daemon` (server-side).~~ **DEFERRED.**
...
- [x] `test/acceptance.sh` is added to CI (not just runnable locally) and runs every numbered item in PRD §14, exiting 0:
  ...
  - ~~a synthetic `capabilities.status=false` harness exercises the capture-pane fallback and is labeled degraded~~ — **deferred** (PRD §10); list still labels degraded but no fallback runs
