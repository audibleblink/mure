...
Each harness ships hook scripts under `harnesses/<name>/hooks/` that
the harness's native hook system invokes on lifecycle events
(tool-call start/end, turn end, etc.). Those scripts exec
`mure emit …` to push status/result frames to the daemon.

### Graceful Degradation
If a harness cannot supply status or result via hooks, mure falls back
to **`tmux capture-pane`**:
- Status: derived heuristically from pane activity (idle if pane
  output is quiescent for N seconds; working otherwise). Specific
  heuristic TBD in plan.
- Result: on `mure wait`, the final N lines of the captured pane
  buffer are returned with a `degraded: true` marker.

Manifest `capabilities.{status,result}=false` opts a harness into this
fallback explicitly; `mure integration list` shows the degraded mode.
